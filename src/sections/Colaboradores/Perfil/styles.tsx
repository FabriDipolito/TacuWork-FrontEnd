...
import { Autocomplete, MenuItem, Select, styled, TextField, Typography } from "@mui/material";
...
export const PhotoContainer = styled("div")(({ theme }) => {
  return {
    ...
    height: "89px",
    borderRadius: "29px",
    border: `1px solid ${theme.customPalette.primary}`,
    backgroundColor: `${theme.customPalette.Perfil.photo}`,
    marginBottom: "-47px",
    zIndex: 1,
    cursor: "pointer",
    ":hover": {
      boxShadow: "0px 0px 14px rgba(247, 178, 0, 1)",
    }
...
export const EditText = styled(Typography)<GuardarProps>(({ theme, clickeable=true }) => {
  const {
    customTypography: {
      InterMedium,
    },
  ...
  return {
    ...
    color: clickeable ? primary : theme.customPalette.Perfil.personal.label,
  };
});
...
export const InfoBancariaContainer = styled("div")(() => {
  return {
    display: "flex",
    flexDirection: "row",
    justifyContent: "space-between",
...
export const GuardarContainer = styled("div")<GuardarProps>(({ clickeable=false }) => {
  return {
    display: "flex",
    flexDirection: "row",
    alignItems: "center",
...
export const SelectText = styled(Select)<FilterComponentProps>(({ theme, placeholder = true, }) => {
  return {
    ...
    ".MuiInputBase-input": {
      ...
      fontWeight: `${theme.customSizes.Perfil.personal.card.input.fontWeight}`,
      borderRadius: "6px",
      marginTop: placeholder ? "10px" : "10px",
